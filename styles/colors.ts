export const primaryColor = 'firebrick'
export const primaryHoverColor = 'rgb(193, 64, 64)'

export const secondaryColor = 'white'
export const secondaryHoverColor = 'rgba(255, 255, 255, 0.2)'

export const errorRedColor = '#e34d4d'

export const blueTeamColor = 'rgba(48, 141, 193)'
export const redTeamColor = 'rgba(190, 64, 55)'

export const resourceColor = '#877725'
export const resourceBgColor = '#f0eaaf'

export const vitalityColor = '#105881'
export const vitalityBgColor = '#b2ccd7'

export const victoryPointsColor = '#8729b3'
